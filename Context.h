...
// import static com.github.jsonldjava.utils.newMap;
...
public: 
	...
	Context ( Object context, JsonLdOptions opts ) : Context(context.isMap() ? context.obj() : base_t()/*null*/ ) { init ( opts ); 	}
	Context ( JsonLdOptions opts ) { init ( opts ); 	}

private:
	void init ( JsonLdOptions options_ ) {
		options = options_;
		if ( options.getBase().size() ) put ( "@base", options.getBase() );
		termDefinitions = newMap();
	}
...
//			remoteContexts = new ArrayList<String>();
		...
		if ( ! ( localContext.isList() ) ) {
			const Object temp = localContext;
