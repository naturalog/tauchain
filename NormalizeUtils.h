...
private: const UniqueNamer namer;
private: const Map<String, Object> bnodes;
private: const List<Object> quads;
private: const JsonLdOptions options;
...
public: Object hashBlankNodes ( Collection<String> unnamed_ )  {
...
private: static String encodeHex ( const byte[] data ) {
		String rval = "";
...
private: static class Permutator {
	...
	private: const Map<String, Boolean> left;
	...
	public: List<String> next() {
			const List<String> rval = ( List<String> ) JsonLdUtils.clone ( this.list );
			...
			String k = null;
			int pos = 0;
			const int length = this.list.size();
			for ( int i = 0; i < length; ++i ) {
				const String element = this.list.get ( i );
				const Boolean left = this.left.get ( element );
			...
			if ( k == null )
				...
				const int swap = this.left.get ( k ) ? pos - 1 : pos + 1;
