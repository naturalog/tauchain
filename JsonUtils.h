...
class JsonUtils {
	...
	*/
protected: static const String ACCEPT_HEADER = "application/ld+json, application/json;q=0.9, application/javascript;q=0.5, text/javascript;q=0.5, text/plain;q=0.2, */*;q=0.1";
private: static const ObjectMapper JSON_MAPPER = new ObjectMapper();
private: static const JsonFactory JSON_FACTORY = new JsonFactory ( JSON_MAPPER );
private: static DocumentLoader DOCUMENT_LOADER = new DocumentLoader();
	...
	*/
public: static Object fromInputStream ( InputStream input )  {
	...
	/**
	    ...
	    @
	               If there was an IO error during parsing.
	*/
public: static Object fromInputStream ( InputStream input, String enc )  {
	...
	/**
	    ...
	    @
	               If there was an IO error during parsing.
	*/
public: static Object fromReader ( Reader reader )  {
		const JsonParser jp = JSON_FACTORY.createParser ( reader );
		Object rval = null;
		const JsonToken initialToken = jp.nextToken();

		if ( initialToken == JsonToken.START_ARRAY )
			rval = jp.readValueAs ( List.class ); else if ( initialToken == JsonToken.START_OBJECT )
			rval = jp.readValueAs ( Map.class ); else if ( initialToken == JsonToken.VALUE_STRING )
			rval = jp.readValueAs ( String.class ); else if ( initialToken == JsonToken.VALUE_FALSE || initialToken == JsonToken.VALUE_TRUE )
			rval = jp.readValueAs ( Boolean.class ); else if ( initialToken == JsonToken.VALUE_NUMBER_FLOAT
		        || initialToken == JsonToken.VALUE_NUMBER_INT )
			rval = jp.readValueAs ( Number.class ); else if ( initialToken == JsonToken.VALUE_NULL )
			rval = null; else {
...
public: static String toPrettyString ( Object jsonObject ) throws JsonGenerationException,
	IOException {
		const StringWriter sw = new StringWriter();
...
public: static void write ( Writer writer, Object jsonObject ) throws JsonGenerationException,
	IOException {
		const JsonGenerator jw = JSON_FACTORY.createGenerator ( writer );
